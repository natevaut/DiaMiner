...
Shader::Shader()
	: m_vertexShader(0), m_pixelShader(0), m_shaderProgram(0)
{
}
...
bool Shader::CompileShader(const char *filename, GLenum shaderType, GLuint &outShader)
{
	...
	{
		LogManager::GetInstance().LogPart("Full path of shader file: ");
		LogManager::GetInstance().Log(fullPath);
	}
	else
	{
		LogManager::GetInstance().LogPart("Error resolving full path of ");
